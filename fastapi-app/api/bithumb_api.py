...
@router.get("/coins-with-marketcap")
async def get_coins_with_marketcap():
    """시가총액이 포함된 코인 목록 조회"""
    try:
        # 기본 코인 목록 가져오기
        basic_coins = await get_coin_list()
        if basic_coins["status"] != "success":
            return basic_coins
        ...
        # 각 코인에 시가총액 추가
        enhanced_coins = []
        for coin in basic_coins["data"][:50]:  # 상위 50개만 처리 (API 호출 제한)
            market_cap_data = await get_coingecko_market_cap(coin["symbol"])
            market_cap_krw = 0
            if market_cap_data:
                market_cap_krw = market_cap_data["market_cap_usd"] * usd_krw_rate
            
            enhanced_coin = {**coin, "market_cap": round(market_cap_krw, 4)}
            enhanced_coins.append(enhanced_coin)
        
        return {
            "status": "success",
            "data": enhanced_coins,
            "total_count": len(enhanced_coins),
            "exchange_rate": usd_krw_rate,
            "last_updated": datetime.now().isoformat()
        }
