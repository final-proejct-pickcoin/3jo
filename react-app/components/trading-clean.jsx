...
//=======매도 매수 관련
  ...
  const guard = () => {
    if (!user_id) return "user_id(사용자 ID)를 입력하세요.";
  ...
  const handleSelectCoin = async (coin) => {
    ...
    // asset_id가 이미 있으면 바로 사용, 없으면 fetch
    let id = coin.asset_id;
    if (id == null) {
      // 마켓 구분 (BTC, KRW 등) 필요시 수정
      const assetSymbol = `${coin.symbol}-KRW`;
  ...
  const handleBuy = async () => {
    ...
    // asset_id가 최신인지 확인 (selectedCoin이 바뀌었을 때만 setAsset_id가 동작하므로, 혹시라도 null이면 fetch)
    let id = asset_id;
